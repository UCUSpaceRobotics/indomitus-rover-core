...
class TestKinematics(unittest.TestCase):
    ...
    def test_straight_motion(self):
        ...
        vx = 1.0  # m/s
        angles, speeds = self.node.kinematics.ik_full(vx, 0.0, 0.0, self.node.current_angles)
    ...
    def test_spin_in_place(self):
        ...
        vtheta = 1.0  # rad/s
        angles, speeds = self.node.kinematics.ik_full(0.0, 0.0, vtheta, self.node.current_angles)

        self.assertAlmostEqual(angles.fl, -angles.fr, places=3)
        self.assertAlmostEqual(speeds.fl, -speeds.fr, places=3)

    def test_steering_limits(self):
        """Steering limits should be enforced."""
        vtheta = 1.0  # rad/s
        angles, _ = self.node.kinematics.ik_full(0.5, 0.0, vtheta, self.node.current_angles)
