...
class JoystickInterpreterNode(Node):
    ...
    def _on_joy(self, msg: Joy):
        ...
        if pressed:
            self._toggle_beautiful()
        self._prev_beautiful_button = cur

        cur, pressed = _check_btn(
            msg.buttons, self._traffic_red_button, self._prev_traffic_red_button
        )
        if pressed:
        ...
        cur, pressed = _check_btn(
            msg.buttons, self._traffic_yellow_button, self._prev_traffic_yellow_button
        )
        if pressed:
        ...
        cur, pressed = _check_btn(
            msg.buttons, self._traffic_green_button, self._prev_traffic_green_button
        )
        if pressed:
        ...
        cur, pressed = _check_btn(
            msg.buttons, self._traffic_blue_button, self._prev_traffic_blue_button
        )
        if pressed:
    ...
    def _toggle_compact_mode(self):
        ...
        future.add_done_callback(lambda f, desired=target: self._on_compact_mode_result(f, desired))
    ...
    def _toggle_spotlight(self):
        self.get_logger().info(
            f"DEBUG _toggle_spotlight called, pending={self._spotlight_pending}, ready={self._spotlight_client.service_is_ready()}"
        )
        ...
        self._spotlight_client.call_async(req).add_done_callback(
            lambda f: self._on_light_result(
                f, "spotlight", self._spotlight_on, "_spotlight_pending"
            )
        )

    def _toggle_beautiful(self):
        self.get_logger().info(
            f"DEBUG _toggle_beautiful called, pending={self._beautiful_pending}, ready={self._beautiful_client.service_is_ready()}"
        )
        ...
        self._beautiful_client.call_async(req).add_done_callback(
            lambda f: self._on_light_result(
                f, "beautiful", self._beautiful_on, "_beautiful_pending"
            )
        )

    def _send_traffic(self):
        self.get_logger().info(
            f"DEBUG _send_traffic called, pending={self._traffic_pending}, ready={self._traffic_client.service_is_ready()}, R={self._traffic_red} Y={self._traffic_yellow} G={self._traffic_green} B={self._traffic_blue}"
        )
        ...
        self._traffic_client.call_async(req).add_done_callback(lambda f: self._on_traffic_result(f))
