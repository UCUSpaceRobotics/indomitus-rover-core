...
// Hardware limits for Steadywin (IDs 20, 21, 22)
...
/**
 * @brief Packs MIT mode control parameters into an 8-byte payload.
 */
...
/**
 * @brief Builds the CAN frame to send MIT control commands to the Steadywin motor.
 */
    ...
    uint8_t motor_id, float pos_rad, float vel_rps, float kp, float kd, float t_ff)
{
    ...
    // Steadywin control ID format: 0x400 | 0x100 | motor_id
    frame.id = 0x400 | 0x100 | motor_id;
    frame.dlc = 8;
...
/**
 * @brief Builds the CAN frame to disable the Steadywin motor.
 */
