#include "rover_controller/swerve_radius_controller.hpp"
...
PLUGINLIB_EXPORT_CLASS(
    rover_controller::RoverSwerveRadiusController,
...
RoverSwerveRadiusController::RoverSwerveRadiusController()
: controller_interface::ControllerInterface()
{}


controller_interface::CallbackReturn
RoverSwerveRadiusController::on_init()
{
    try {
        declare_parameters();
...
controller_interface::CallbackReturn
RoverSwerveRadiusController::on_configure(const rclcpp_lifecycle::State & /*previous_state*/)
{
    if (!read_parameters()) {
        return controller_interface::CallbackReturn::ERROR;
    }
...
controller_interface::CallbackReturn
RoverSwerveRadiusController::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
{
...
controller_interface::CallbackReturn
RoverSwerveRadiusController::on_deactivate(const rclcpp_lifecycle::State & /*previous_state*/)
{
    if (drive_handles_) {
...
RoverSwerveRadiusController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration
RoverSwerveRadiusController::state_interface_configuration() const
{
    controller_interface::InterfaceConfiguration cfg;
    cfg.type = controller_interface::interface_configuration_type::INDIVIDUAL;
...
RoverSwerveRadiusController::update(
...
void RoverSwerveRadiusController::declare_parameters()
{
    auto node = get_node();
...
bool RoverSwerveRadiusController::read_parameters()
{
    auto node = get_node();
...
RoverSwerveRadiusController::steer_command_interface_names() const
{
...
std::vector<std::string>
RoverSwerveRadiusController::drive_command_interface_names() const
{
    std::vector<std::string> names;
...
std::vector<std::string>
RoverSwerveRadiusController::steer_state_interface_names() const
{
    std::vector<std::string> names;
    for (const auto & joint : steer_joint_names_) {
        names.push_back(joint + "/" + hardware_interface::HW_IF_POSITION);
...
bool RoverSwerveRadiusController::assign_interfaces()
{
...
void RoverSwerveRadiusController::read_current_angles()
{
...
void RoverSwerveRadiusController::read_measured_angles()
{
   if (!steer_handles_) { return; }
   for (std::size_t i = 0; i < NUM_WHEELS; ++i) {
#if defined(JAZZY_OR_LATER)
...
void RoverSwerveRadiusController::write_steer_commands(const WheelData & angles)
{
    if (!steer_handles_) { return; }
    for (std::size_t i = 0; i < NUM_WHEELS; ++i) {
...
void RoverSwerveRadiusController::write_drive_commands(
...
double RoverSwerveRadiusController::step_angle(
    double current,
    double target,
    double dt,
    double rate) const
...
void RoverSwerveRadiusController::on_set_compact_mode(
...
bool RoverSwerveRadiusController::cmd_vel_timed_out(const rclcpp::Time & now) const
{
