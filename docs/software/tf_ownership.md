...
* **`zed_node`** (`zed_wrapper`) — Visual odometry TF broadcasting is intentionally disabled: `pos_tracking.publish_tf: false` and `pos_tracking.publish_map_tf: false` in `config/zed2i.yaml`. It provides the `/zed2i/odom` topic consumed as `odom1` by the EKF **in the real hardware configuration only**, ensuring it does not conflict with `ekf_filter_node` or `slam_toolbox`. Its `/zed2i/imu/data` topic is consumed directly by `tilt_broadcaster_node` instead — not a conflict, different TF edge. **Exception:** `sensors.publish_imu_tf: true` — see the `zed2i_left_camera_frame -> zed2i_imu_link` row above.
