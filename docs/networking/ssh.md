...
1. SSH into the Jetson (e.g., via the Hotspot first) or use display and keybord connected to it.
...
If the Jetson is connected to your laptop via Ethernet and a router at the same time, it may incorrectly try to route all its internet traffic through your laptop instead of the router.

To fix this, tell the Jetson to use the Ethernet connection strictly for local access, and never as a default internet gateway. Run this command on the Jetson:
...
> **Important:** From now on you **must** use `ETH0` port only to connect Jetson to the laptop and `ETH1` for connecting Jetson to the router. If you plug ethernet cable from the router to the `ETH0` there will be no internet access on the Jetson. 
...
> **Note:** The same command will work for both ethernet and hotspot connection. If you connected to Jetson with both ethernet cable and hotspot the ethernet will win when running the comand above and you will ssh into the Jetson via ethernet.